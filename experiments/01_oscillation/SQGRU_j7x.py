...
from VQC_j7x import temp_Variation, temp_VQC
...
class QGRUCirc:
    def __init__(self, input_dim, hidden_dim, number_of_layers=2,
        ...
        # number of weights
        self.num_weights = self.n_enc_weights + self.n_hid_weights + lay_n_w + self.gate_n_weights +        self.n_out_weights + self.n_hout_weights
        
    def __call__(self, inputs, weights, measure_basis='z', probs_out=False):
...
#         hiddens = hiddens.squeeze()
        ...
        # ending
        if measure_basis.lower() == 'x':
            for i in range(self.qN_e):
                qml.Hadamard(wires=i)
            
        if probs_out:
            return qml.probs(wires=range(self.pOut_wires))
        else:
            return [qml.expval(qml.PauliZ(i)) for i in range(self.qN_e)]
...
class SQGRU:
    ...
    def __subCell__(self, X, hidden, params):
        inp = jnp.concatenate((X, hidden), axis=0)
        out1 = self.qnode(inp, params, measure_basis='z')
        out2 = self.qnode(inp, params, measure_basis='x')
        
        out = jnp.array([out1, out2]).T
    ...
    def __subCell2__(self, X, hidden, params):
        ...
        inp = jnp.concatenate((X, hidden), axis=0)
        out = self.qnode(inp, params, measure_basis='z', probs_out=True)
        return out
    
    def __call__(self, Xs_b, hiddens, params):
...
#         Hs = jnp.concatenate((Hs1, Hs2), axis=1)
    ...
    def draw_circuit(self, measure_basis='z', probs_out=False, expansion_strategy=None,
                     style='black_white', fontsize=24):
        qml.drawer.use_style(style)
        inp = jnp.ones(self.in_dim + self.hidden_dim)
        params = jnp.array(range(self.num_weights))
        # self.circuit_fig, _ = qml.draw_mpl(self.qnode, expansion_strategy=expansion_strategy,
        #                                    fontsize=fontsize)(inp, params, measure_basis=measure_basis,
        #                                                       probs_out=probs_out)
        self.circuit_fig, _ = qml.draw_mpl(self.qnode, expansion_strategy=expansion_strategy,
                                           )(inp, params, measure_basis=measure_basis,
                                                              probs_out=probs_out)
...
def qgru(in_dim, hidden_dim, number_of_layer=2, enc_layers=0, enc_reupload=1,
    ...
    def apply_fun(params, Xs, **kwargs):
        ...
        h = jnp.zeros([jnp.shape(Xs)[0], gruCell.hidden_dim]) # maybe ones?
        
        def apply_fun_scan01(params, hidden, X):
            out, _, new_hidden = gruCell(X, hidden, params)
            if out_fun != None:
                out = out_fun(out)
            return new_hidden, out
        
        def apply_fun_scan02(params, hidden, X):
        ...
        # predictiton part
        def apply_fun_pred01(params, hidden_X, X):
            hidden = hidden_X[0]
            X = hidden_X[1]
            out, _, new_hidden = gruCell(X, hidden, params)
            if out_fun != None:
                out = out_fun(out)
            new_hidden_X = [new_hidden, out]
            return new_hidden_X, out
        
        def apply_fun_pred02(params, hidden_X, X):
            hidden = hidden_X[0]
            X = hidden_X[1]
            _, out, new_hidden = gruCell(X, hidden, params)
            if out_fun != None:
        ...
        if all_qubits_out:
            f = partial(apply_fun_scan01, params)
            f2 = partial(apply_fun_pred01, params)
        else:
            f = partial(apply_fun_scan02, params)
            f2 = partial(apply_fun_pred02, params)
            
        h_new, out = lax.scan(f, h, Xs)
        ...
        if p_len > 0:
            ...
            if Xs[0].shape != out[-1].shape:
                raise ValueError(f"The output shape {out[-1].shape} is not equal to the input shape {Xs[0].shape}. Please check the output function for qgru.")
            
            h2 = [h_new, out[-1]]
            new_hid_x, out2 = lax.scan(f2, h2, jnp.zeros(p_len))
            h_new = new_hid_x[0]
        
        
        if out_type == 0:
            # output all steps (expected values)
            out = jnp.moveaxis(out, 1, 0)
            if pred_length > 1:
                out2 = jnp.moveaxis(out2, 1, 0)
                out = jnp.concatenate([out, out2], axis=1)
        elif out_type == -1:
            # only output the last step (expected values)
            if pred_length > 1:
                out = out2[-1]
            else:
                out = out[-1]
            ...
            if pred_length > 1:
                X_last = out2[-1]
            # probability out
            # only output the last step (probability)
