...
# coding: utf-8
#
# run_QLSTM_ces_MNIST.py
# ---------------------------------------------------------------------------
# QLSTM (Ceschini-style reformulation) MNIST experiment as a single callable
# function, parameterized by (n_qubits, n_layers). Mirrors run_QGRU_MNIST.py
# exactly -- see that file's header for the general pattern.
#
# Usage (interactively, e.g. in a notebook):
#   from run_QLSTM_ces_MNIST import run_qlstm_ces_experiment
#   run_qlstm_ces_experiment(n_qubits=4, n_layers=5)
#
# Usage (batch, from the command line):
#   python3 run_QLSTM_ces_MNIST.py 4 5 --n_steps 2000
# ---------------------------------------------------------------------------
...
from QLSTM_ces_lit_j8 import qlstm_ces_lit, qlstm_ces_lit_param_count
from QGRU_lit_j8 import QGRUGateCirc  # QLSTM_ces_lit_j8 reuses this gate circuit
...
READOUT_DIM = 2
SEED = 2
RESULTS_CSV = 'results_QLSTM_ces_MNIST.csv'
...
def draw_circuit(n_qubits, n_layers, save_dir=CIRCUIT_DIR):
    """Draw one gate VQC (forget/input/candidate/output share this structure)."""
    ...
    png_path = os.path.join(save_dir, f'QLSTMces_q{n_qubits}_l{n_layers}.png')
...
def run_qlstm_ces_experiment(n_qubits, n_layers, hidden_dim=DEFAULT_HIDDEN_DIM,
                              N_STEPS=500, NUM_SEEDS=1,
                              draw=True, verbose=True, results_csv=RESULTS_CSV):
    """Run one QLSTM (Ceschini-style) MNIST experiment for a given
    (n_qubits, n_layers), with hidden_dim defaulting to 8 (matching QRU's
    own hidden dimension). Uses readout_mode='slice' (hidden_dim >=
    readout_dim=2, output is the first 2 components of the final hidden
    state -- no extra readout parameters, mirroring QRU's own readout
    convention). Returns a dict with n_params, test_acc, elapsed_seconds,
    checkpoint_name, circuit_png (if draw=True)."""
    config_tag = f'h{hidden_dim}_q{n_qubits}_l{n_layers}'
    checkpoint_name = f'QLSTMces_MNIST_{config_tag}'

    n_params = qlstm_ces_lit_param_count(IN_DIM, hidden_dim, n_qubits, n_layers,
    ...
    init_fun, apply_fun = qlstm_ces_lit(IN_DIM, hidden_dim, n_qubits, n_layers,
                                         readout_dim=READOUT_DIM, readout_mode='slice')
...
if __name__ == '__main__':
    ...
    run_qlstm_ces_experiment(args.n_qubits, args.n_layers, hidden_dim=args.hidden_dim,
